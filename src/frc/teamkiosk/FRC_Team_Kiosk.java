...
import frcradiokiosk.FRCRadio.Mode;
import frcradiokiosk.RadioFactory.RadioOptions;
...
public class FRC_Team_Kiosk extends JFrame implements IKioskShell {
   ...
   public FRC_Team_Kiosk() {
      ...
      for (RadioOptions choice : RadioOptions.values()) {
      ...
      for (Mode choice : Mode.values()) {
         if (choice != Mode.BRIDGE24) {
      ...
      this.jModeComboBox.setSelectedItem(Mode.AP24.getLongString());
   ...
   public void startup() {
      try {
         ...
         logger.error("failed to initialize host computer network settings", var3);
   ...
   private void jChangeNicMenuItemActionPerformed(ActionEvent evt) {
      try {
         ...
         logger.error("Failed to refresh and retreive network interface list", var4);
   ...
   private void jEventConfigureButtonActionPerformed(ActionEvent evt) {
      try {
         ...
         this.showErrorMessage(var6.getUserMessage());
         logger.error("Error starting program sequence: " + var6.getMessage(), var6);
      }
   }
   ...
   private void jFmslCheckBoxMenuItemActionPerformed(ActionEvent evt) {
      if (this.jFmslCheckBoxMenuItem.isSelected()) {
         ...
         if (cancelled) {
            ...
            this.jModeComboBox.setSelectedItem(Mode.BRIDGE.getLongString());
